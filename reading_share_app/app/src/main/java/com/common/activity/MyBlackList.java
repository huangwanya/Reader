...
public class MyBlackList extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		myApp = (MyApp) MyBlackList.this.getApplication();
...
searchbtn.setOnClickListener(new OnClickListener() {
	...
	public void add(View view) {
		Intent intent =new Intent(MyBlackList.this,FriendsAll.class);
		...
		new Thread() {
			public void run() {
				try {
					...
					String url = HttpUtil.URL_MYBLACKLIST+myApp.getLoginKey();
					String result = null;
	...
	private Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			if (msg.what == 0x127) {
				//
				Toast.makeText(MyBlackList.this, "操作成功", Toast.LENGTH_SHORT).show();
				;
				getGameXD("");
			}
			if (msg.what == 0x128) {
				//
				Toast.makeText(MyBlackList.this, "操作失败", Toast.LENGTH_SHORT).show();
				;
				
			}
			if (msg.what == 0x129) {
				//
				Toast.makeText(MyBlackList.this, "抱歉,请不要重复操作", Toast.LENGTH_SHORT).show();
				;
		...
		new Thread() {
			public void run() {
				try {
					...
					String url = HttpUtil.URL_DELBLACKLIST + "friends.userid1="+userid1+"&friends.userid2="+userid2;
					String result = null;
					result = HttpUtil.queryStringConnectForPost(url);
...
/*	public void check_creator(final String goods_id) {
	...
	private class GameAdapter extends BaseAdapter {
		...
		private GameAdapter() {
			inflater = LayoutInflater.from(MyBlackList.this);
		}
...
//			if(basemarkBeans.get(position).getStatus().equals("0")){
//				if(basemarkBeans.get(position).getUserid2().equals(myApp.getLoginKey())){
//					new AlertDialog.Builder(MyBlackList.this).setTitle("提示")
...
//			}else{
				new AlertDialog.Builder(MyBlackList.this).setTitle("提示")
		        .setMessage("根据提示进行操作").setNegativeButton("取消", new DialogInterface.OnClickListener() {
...
//		            	delete(basemarkBeans.get(position).getUserid1(),basemarkBeans.get(position).getUserid2());
		        ...
		        }).setPositiveButton("移除黑名单", new DialogInterface.OnClickListener(){
		            @Override  
		            public void onClick(DialogInterface dialog, int which) {  
		                // TODO Auto-generated method stub
...
//			}
			...
			Toast.makeText(MyBlackList.this, "请登录后进行操作", Toast.LENGTH_SHORT).show();
			Intent intent = new Intent(MyBlackList.this,Login.class);
			startActivity(intent);
		}
